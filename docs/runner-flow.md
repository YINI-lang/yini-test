...
At the end of a run, the runner prints a terminal summary table showing the adapter name, the `yini-test-suite` version, the requested test suite, the declared YINI spec revision, each suite/mode group, passed/failed/total counts, and duration. It also prints the one-line summary in this form:
```txt
